...
export class MandatService {
  ...
  getType(): Observable<TypeMandat[]> {
    return this.http.get<TypeMandat[]>(
      `${this.hosturlname}api/v2/mandate_types.json`
    );
  }
  ...
  getMandats(id): Observable<Mandat[]> {
    return this.http.get<Mandat[]>(
      `${this.hosturlname}api/v2/projects/${id}/mandates.json`
    );
  }
  ...
  getMandat(id): Observable<Mandat> {
    return this.http.get<Mandat>(
      `${this.hosturlname}api/v2/mandates/${id}.json`
    );
  }
  ...
  getAllMandats(params: any): Observable<Mandat[]> {
    ...
    return this.http.get<Mandat[]>(`${this.hosturlname}api/v2/mandates`, {
      params: data,
    });
  }
  /**
