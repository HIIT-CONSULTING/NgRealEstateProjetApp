...
export class MandatAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.mandatService
      ...
      .subscribe((res) => {
        ...
        this.lastPage = res['hydra:view']['hydra:last']
          ? res['hydra:view']['hydra:last'].split('page=')[1]
          : 1;
      });
  }
  onSubmit() {
  ...
  refus(id: number) {
    this.mandatService
      .updateMandat({ status: 'refused' }, id)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((res) => {
        this.mandatService
          .getAllMandats(this.form.value)
          ...
          .subscribe(
            (res) => {
              this.snackBar.open('le mandat est refusé!', '', {
                duration: 1000,
  ...
  accept(id: number) {
    this.mandatService
      .updateMandat({ status: 'accepted' }, id)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((res) => {
        this.mandatService
          .getAllMandats(this.form.value)
          ...
          .subscribe(
            (res) => {
              this.snackBar.open('le mandat est accepté!', '', {
                duration: 1000,
                panelClass: ['blue-snackbar'],
                verticalPosition: 'top',
                horizontalPosition: 'end',
