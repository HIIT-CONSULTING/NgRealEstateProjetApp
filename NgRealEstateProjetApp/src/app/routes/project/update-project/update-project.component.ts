...
import { City, Contact, Country, Project } from '@shared/models/Agent.model';
...
export class UpdateProjectComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.form
      ...
      .subscribe((value) => {
        value
          ? this.form.get('property.keysNumber').enable()
          : this.form.get('property.keysNumber').disable();
      });
  }
  ...
  ngOnInit(): void {
    ...
    this.projectService.getProject(this.id).subscribe((project) => {
      ...
      this.city$ = this.sponsorService.getCitys(
        project.property.address.country.id
      );
      if (project != {}) {
  ...
  OnClick() {
    if (this.form.get('contact').value != null) {
      this.show = true;
      this.contactService
        .getContact(this.form.get('contact').value)
        .subscribe((contact) => {
  ...
  formatDate() {
    if (this.form.get('property.constructionYear').value != '') {
      this.form
        .get('property.constructionYear')
        .setValue(
          moment(this.form.get('property.constructionYear').value).format(
            'YYYY-MM-DD'
          )
        );
    }
    if (this.form.get('property.dateAvailability').value != '') {
      this.form
        .get('property.dateAvailability')
        .setValue(
          moment(this.form.get('property.dateAvailability').value).format(
            'YYYY-MM-DD'
          )
        );
    }
  ...
  onSubmit() {
    ...
    this.projectService.updateProject(this.form.value, this.id).subscribe(
      (user) => {
        user = user;
        this.snackBar.open('le projet est modifié avec succès!', '', {
          duration: 1000,
