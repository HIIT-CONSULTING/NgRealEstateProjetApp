...
import { Candidate, Agent, Gender, City, Country, Contact, Contact_type } from '@shared/models/Agent.model';
  ...
  onSubmit() { 
   ...
   this.contact['gender']= this.form.get('gender').value; 
    ...
    this.contact['address']= this.form.get('address').value; 
     ...
     this.contactService.updateContact(this.contact,this.id).subscribe(user => { 
     user = user; 
     }); 
